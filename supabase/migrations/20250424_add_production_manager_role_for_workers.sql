-- ═══════════════════════════════════════════════════════════════════
-- Production Manager Role 및 생산관리 테이블 추가 (workers 버전)
-- ═══════════════════════════════════════════════════════════════════
...
-- 4. production_entries RLS 정책 (workers 테이블 기준)
...
USING (
  -- 본인 데이터 접근
  user_id = auth.uid()
  -- production_manager 또는 admin 역할 확인 (workers 테이블에서)
  OR EXISTS (
...
-- 5. production_expenses RLS 정책 (workers 테이블 기준)
...
-- 6. 인덱스 생성
CREATE INDEX IF NOT EXISTS idx_production_entries_user_date ON production_entries(user_id, entry_date);
CREATE INDEX IF NOT EXISTS idx_production_entries_product ON production_entries(product_type);
CREATE INDEX IF NOT EXISTS idx_production_expenses_user_date ON production_expenses(user_id, expense_date);
CREATE INDEX IF NOT EXISTS idx_production_expenses_type ON production_expenses(expense_type);
