...
/* ─── Constants ─── */
export const INOPNC_COMPANY = '(주)이노피앤씨'

/* ─── Signup Metadata Types ─── */
export type SignupRole = Exclude<Role, 'admin'>

export interface BaseSignupMetadata {
  name: string
  phone?: string
  affiliation?: string
  title?: string
...
export interface SignupMetadata extends BaseSignupMetadata {
  role: SignupRole
  company: string
  site_ids?: string[]
}
...
export async function fetchWorkerProfile(
  ...
  const { data, error } = await supabase
    .from('workers')
    .select('id, email, name, role, company, phone, daily, affiliation, site_ids, title')
    .eq('id', userId)
    .single()
...
export async function signUpWithEmail(
  ...
  if (data.user) {
    ...
    if (metadata.role === 'partner') {
      insertData.company = metadata.company
    } else {
      insertData.company = INOPNC_COMPANY
    }

    await supabase.from('workers').upsert(insertData)
  }
