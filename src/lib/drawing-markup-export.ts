...
import type { DrawingMarkupPreviewDocument, DrawingMarkupMark } from '@/components/preview/reports/drawing-markup-preview-types'
...
export function generateDrawingMarkupCsv(options: DrawingMarkupExportOptions): string {
  ...
  const rows: string[][] = [
    ['문서 제목', '현장명', '작업일', '페이지', '마킹 유형', '면적', `면적 (${areaUnit})`],
  ]
...
export async function createDrawingMarkupPdfBlob(
  ...
  const htmlContent = `
    <div class="container" style="width: ${containerWidthPx}px; font-family: 'Malgun Gothic', '맑은 고딕', 'Apple SD Gothic Neo', sans-serif;">
      <h1 class="title">${escapeHtml(document.title || '도면마킹 비교표')}</h1>
      <div class="meta">
      ...
      <h2 class="section-title">페이지별면적 요약</h2>
    ...
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { padding: 40px; background: white; color: #1a1a1a; font-size: 14px; line-height: 1.6; }
        .title { font-size: 24px; font-weight: bold; text-align: center; margin-bottom: 16px; color: #1a1a1a; }
        .meta { margin-bottom: 20px; font-size: 13px; color: #666; text-align: center; }
        .meta div { margin-bottom: 4px; }
        .section-title { font-size: 16px; font-weight: bold; margin: 24px 0 12px 0; padding-bottom: 8px; border-bottom: 2px solid #dc2626; color: #1a1a1a; }
        .summary-table, .detail-table { width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 12px; }
        .summary-table th, .detail-table th { background: #f5f5f5; padding: 10px 8px; text-align: left; font-weight: bold; border: 1px solid #e5e5e5; }
        .summary-table td, .detail-table td { padding: 8px; border: 1px solid #e5e5e5; }
        .total-row { font-weight: bold; background: #fff3f3; }
        .page-section { margin-bottom: 24px; padding: 16px; background: #fafafa; border-radius: 4px; }
        .page-header { font-size: 14px; font-weight: bold; margin-bottom: 12px; color: #1a1a1a; }
        .no-marks { color: #999; font-style: italic; text-align: center; padding: 20px; }
      </style>
    </head>
