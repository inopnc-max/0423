...
function WorklogListView({
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
        <label className="block">
          <span className="mb-1 flex items-center gap-2 text-sm font-medium text-[var(--color-text-sub)]">
            현장 선택
          </span>
          <select
            value={selectedSiteId}
            onChange={e => onSiteSelect(e.target.value)}
            className="w-full rounded-xl border border-[var(--color-border)] px-3 py-3 text-sm outline-none focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var(--color-accent-ring)]"
          >
      ...
      {loadingLogs ? (
        <div className="flex h-32 items-center justify-center">
          <span className="text-sm text-[var(--color-text-sub)]">로딩 중...</span>
        </div>
      ) : !selectedSiteId ? (
        <div className="rounded-2xl bg-[var(--color-bg-surface)] p-8 text-center text-sm text-[var(--color-text-sub)] shadow-sm">
        ...
        <div className="space-y-2">
          ...
          {logs.map(log => (
            <button
              ...
              className="w-full rounded-2xl bg-[var(--color-bg-surface)] p-4 text-left shadow-sm transition hover:bg-[var(--color-bg-highlight)]"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--color-accent-soft)]">
                    <ClipboardList className="h-5 w-5 text-[var(--color-accent-strong)]" strokeWidth={1.9} />
                  </div>
                  <div>
                    <div className="font-medium text-[var(--color-text-main)]">
                      {format(new Date(log.work_date), 'yyyy년 MM월 dd일')}
                    </div>
                    <div className="mt-0.5 flex items-center gap-2 text-xs text-[var(--color-text-sub)]">
  ...
  return (
    <div className="space-y-4">
      <button
        type="button"
        onClick={onBack}
        className="flex items-center gap-2 text-sm font-medium text-[var(--color-accent-strong)] transition hover:text-[var(--color-primary-strong)]"
      >
      ...
      <div className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-[var(--color-primary-strong)]">
            {format(new Date(log.work_date), 'yyyy년 MM월 dd일')}
          </h2>
          <StatusBadge status={log.status} />
        </div>
      ...
      <div className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-[var(--color-primary-strong)]">출역 인원</h3>
          <span className="ml-auto rounded-full bg-[var(--color-accent-soft)] px-3 py-1 text-sm font-semibold text-[var(--color-accent-deep)]">
            {totalWorkers}명
          </span>
        </div>
        ...
        {log.worker_array.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {log.worker_array.map((worker, i) => (
              <span
                key={`${worker.name}-${i}`}
                className="inline-flex items-center gap-1 rounded-full bg-[var(--color-accent-soft)] px-3 py-2 text-sm text-[var(--color-accent-deep)]"
              >
      ...
      <div className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-[var(--color-primary-strong)]">작업 항목</h3>
          <span className="ml-auto text-sm text-[var(--color-text-sub)]">
        ...
        {log.task_tags.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {log.task_tags.map(tag => (
              <span
                key={tag}
                className="inline-flex items-center rounded-full bg-[var(--color-accent-soft)] px-3 py-2 text-sm font-medium text-[var(--color-text-sub)]"
              >
                {tag}
              </span>
            ))}
      ...
      <div className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-[var(--color-primary-strong)]">자재 투입</h3>
          <span className="ml-auto text-sm text-[var(--color-text-sub)]">
          ...
          <div className="space-y-2">
            {log.material_items.map((item, i) => (
              ...
              >
                ...
                <span className="text-[var(--color-text-sub)]">수량: {item.quantity}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-[var(--color-primary-strong)]">사진 및 도면</h3>
        </div>
...
export default function WorklogPage() {
  ...
  if (isPartnerUser) {
    if (selectedLog) {
      return (
        <div className="space-y-4 p-4">
          <div>
            <h1 className="text-xl font-bold text-[var(--color-primary-strong)]">일지 상세</h1>
    ...
    return (
      <div className="space-y-4 p-4">
        <div>
          <h1 className="text-xl font-bold text-[var(--color-primary-strong)]">일지 조회</h1>
          <p className="mt-1 text-sm text-[var(--color-text-sub)]">
...
function WorklogEditorView({
  ...
  return (
    <div className="space-y-4 p-4 pb-28">
      <div>
        <h1 className="text-xl font-bold text-[var(--color-primary-strong)]">작업일지</h1>
        <p className="mt-1 text-sm text-[var(--color-text-sub)]">
      ...
      <div className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <label className="block">
            <span className="mb-1 block text-sm font-medium text-[var(--color-text-sub)]">작업일</span>
            <input
              type="date"
              value={selectedDate}
        ...
        {existingLog?.status && (
          <div className="mt-3 rounded-xl bg-[var(--color-accent-soft)] px-4 py-3 text-sm text-[var(--color-accent-strong)]">
      ...
      {!selectedSite ? (
        <div className="rounded-2xl bg-[var(--color-bg-surface)] p-8 text-center text-[var(--color-text-sub)] shadow-sm">
        ...
        <>
                ...
                <button
                  ...
                  className={`rounded-2xl border px-4 py-3 text-left transition ${
                    active
                      ...
                      : 'border-[var(--color-border)] bg-[var(--color-bg-surface)] hover:bg-[var(--color-bg-highlight)]'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className="rounded-full bg-[var(--color-bg-surface)] p-2 shadow-sm">
                      <Icon className="h-[18px] w-[18px] text-[var(--color-accent)]" strokeWidth={1.9} />
                    </div>
                    <div>
                      <div className="text-sm font-semibold text-[var(--color-text-main)]">{label}</div>
          ...
          <section className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-[var(--color-primary-strong)]">1. 출역 인원 및 공수</h2>
              <span className="rounded-full bg-[var(--color-accent-soft)] px-3 py-1 text-sm font-semibold text-[var(--color-accent-deep)]">
                {totalWorkers}명
              </span>
            </div>
            <div className="mb-4 flex gap-2">
              <input
                ...
                placeholder="작업자 이름"
                className="flex-1 rounded-xl border border-[var(--color-border)] px-3 py-3 text-sm outline-none focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var(--color-accent-ring)]"
              />
              <button
                type="button"
                onClick={addWorker}
                className="rounded-xl bg-[var(--color-primary-strong)] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[var(--color-primary-hover)]"
              >
                추가
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {workerArray.map(worker => (
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-[var(--color-accent-soft)] px-3 py-2 text-sm text-[var(--color-accent-deep)]"
                >
                  <span>{worker.name} ({worker.count})</span>
                  <span className="text-[var(--color-accent-deep)]">삭제</span>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
            <h2 className="mb-4 text-lg font-semibold text-[var(--color-primary-strong)]">2. 상세 작업 및 체크리스트</h2>
            <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className={`rounded-full px-3 py-2 text-sm font-medium transition ${
                    taskTags.includes(tag)
                      ...
                      : 'bg-[var(--color-accent-soft)] text-[var(--color-text-sub)] hover:bg-[var(--color-bg-highlight)]'
                  }`}
                >
                  {tag}
                </button>
          ...
          <section className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
            <h2 className="mb-4 text-lg font-semibold text-[var(--color-primary-strong)]">3. 자재 투입 현황</h2>
            <div className="mb-4 flex gap-2">
              <input
                type="text"
              ...
              <input
                ...
                onFocus={() => setActiveSection('materials')}
                placeholder="수량"
                className="w-24 rounded-xl border border-[var(--color-border)] px-3 py-3 text-sm outline-none focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var(--color-accent-ring)]"
              />
              <button
                type="button"
                onClick={addMaterial}
            ...
            <div className="space-y-2">
              ...
              {materialItems.map((material, index) => (
                <div key={`${material.name}-${index}`} className="flex items-center justify-between rounded-xl bg-[var(--color-bg)] px-4 py-3 text-sm">
                  <span className="font-medium text-[var(--color-text-main)]">{material.name}</span>
                  <div className="flex items-center gap-3">
                    <span className="text-[var(--color-text-sub)]">{material.quantity}</span>
          ...
          <section className="rounded-2xl bg-[var(--color-bg-surface)] p-4 shadow-sm">
            <h2 className="mb-4 text-lg font-semibold text-[var(--color-primary-strong)]">4. 현장 사진 및 도면</h2>
            <p className="text-sm text-[var(--color-text-tertiary)]">
              현장 상세 화면에서 사진 및 도면을 확인하세요.
            </p>
          </section>
      ...
      {selectedSite && (
        <div className="fixed inset-x-0 bottom-16 border-t border-[var(--color-border)] bg-[var(--color-bg-surface)]/95 px-4 py-4 backdrop-blur">
          <div className="mx-auto flex max-w-3xl gap-3">
            <button
              type="button"
              onClick={() => handleSave('draft')}
              disabled={saving}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-[var(--color-border)] px-4 py-3 text-sm font-semibold text-[var(--color-text-main)] transition hover:bg-[var(--color-bg-highlight)] disabled:opacity-60"
            >
              임시저장
            </button>
            <button
              ...
              disabled={saving}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[var(--color-primary-strong)] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[var(--color-primary-hover)] disabled:opacity-60"
            >
              {saving ? '저장 중...' : '승인 요청'}
            </button>
          </div>
